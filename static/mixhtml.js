...
async function mixhtml(el = false) {
  ...
  if (el.hasAttribute("mix-get")) {
    url = el.getAttribute("mix-get");
  }
  if (el.hasAttribute("mix-post")) {
    url = el.getAttribute("mix-post");
  }
  if (el.hasAttribute("mix-put")) {
    url = el.getAttribute("mix-put");
  }
  if (el.hasAttribute("mix-delete")) {
    url = el.getAttribute("mix-delete");
  }
  if (url == "") {
    cl(`mix-method missing, therefore url not found`);
    return;
  }
  cl(`url: ${url}`);
...
async function mix_fetch_data(el) {
  ...
  if (method == "post" || method == "put") {
    if (!el.getAttribute("mix-data")) {
      cl(`error : mix_fetch_data() mix-data missing`);
      return;
    }
    if (!document.querySelector(el.getAttribute("mix-data"))) {
      cl(`error - mix-data element doesn't exist`);
      return;
    }
...
function process_template(mix_url) {
  ...
  document.querySelectorAll("template[mix-target]").forEach((template) => {
    ...
    if (!template.getAttribute("mix-target")) {
      console.log(`process_template() - error - mix-target missing`);
      return;
    }
    console.log(`ok : mix() the response data will affect '${template.getAttribute("mix-target")}'`);
    if (!document.querySelector(template.getAttribute("mix-target"))) {
      console.log(`process_template() - error - mix-target is not in the dom`);
      return;
    }

    let position = "innerHTML";
    if (template.hasAttribute("mix-before")) {
      position = "beforebegin";
    }
    if (template.hasAttribute("mix-after")) {
      position = "afterend";
    }
    if (template.hasAttribute("mix-top")) {
      position = "afterbegin";
    }
    if (template.hasAttribute("mix-bottom")) {
      position = "beforeend";
    }
    if (template.hasAttribute("mix-replace")) {
      position = "replace";
    }
    ...
    if (!template.getAttribute("mix-push-url")) {
      cl(`process_template() - optional - mix-push-url not set`);
    }
    template.remove();
    mix_convert();
